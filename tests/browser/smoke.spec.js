...
test('loads visualisation JSON generated by the installed CLI', async ({ page }) => {
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'grapetree-json-'));
  const treePath = path.join(fixtureDir, 'tree.nwk');
  const metadataPath = path.join(fixtureDir, 'metadata.tsv');
  fs.writeFileSync(treePath, '(json_alpha:1,json_beta:2);');
  fs.writeFileSync(metadataPath, 'ID\tCountry\njson_alpha\tUK\njson_beta\tFR\n');
  const document = execFileSync(
    process.env.GRAPETREE_CLI || 'grapetree',
    ['--json', '--treefile', treePath, '--meta', metadataPath],
    { encoding: 'utf8' }
  );

  await page.goto('/');
  await page.evaluate(jsonText => loadTreeText(jsonText), document);

  await expect.poll(async () => page.evaluate(() => (
    the_tree ? the_tree.force_nodes.map(node => node.id).sort() : []
  ))).toEqual(['_hypo_0', 'json_alpha', 'json_beta']);
  expect(await page.evaluate(() => the_tree.metadata.json_alpha.Country)).toBe('UK');
  expect(await page.evaluate(() => the_tree.display_category)).toBe('Country');
});
