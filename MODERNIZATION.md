...
- 13 Playwright/Chromium tests pass. Twelve run against the Flask app, covering Newick
  ...
  The thirteenth loads the separate static browser backend and calculates an
  Edmonds branching in a Web Worker/WASM module.
...
5. Join profile parsing, distance calculation, and MSTree/MSTreeV2 assembly to
   the browser Worker API, then load the result into the established visualiser.
