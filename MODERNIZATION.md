...
- Added a no-server profile pipeline for browser MSTree and MSTreeV2, including
  ...
  cover the basic topology and full issue #82 replicate behaviour. NJ and
  RapidNJ still require verified source ports before browser parity is complete.

## Current verification

- Clean wheel and source distribution build successfully with Hatchling.
- 84 Python tests pass on Python 3.12.
- 20 Playwright/Chromium tests pass. Twelve run against the Flask app, covering Newick
  ...
  Eight exercise the separate static browser backend: real MSTree/MSTreeV2
...
5. Verify canonical RapidNJ/FastME sources and licences, then add browser NJ
   backends only when they pass the shared pairwise-distance fixtures.
