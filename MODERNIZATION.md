...
- 23 Playwright/Chromium tests pass. Twelve run against the Flask app, covering Newick
...
- The browser-only flavour now presents the established full GrapeTree
  visualiser rather than a separate prototype form. Its normal **Load Files**
  and method-selection flow is retained, while profile calculations are
  redirected to the local Worker. The Flask application remains unchanged as
  the primary server-backed product, and the browser entry point makes no
  `/maketree` request.
- A realistic 67-profile, 3,002-locus cgMLST input uncovered two exact-parity
  boundaries in MSTreeV2: NumPy float32 formatting before Edmonds and a legacy
  branch-recraft candidate tie. The Worker now reproduces both established
  behaviours and generated byte-identical Newick for that input. A compact
  synthetic regression protects those boundaries without committing the user
  dataset.
- A manually managed test deployment is available at
  <https://grapetree-browser-preview.vercel.app>. It is deployed directly with
...
- Final local verification after the integrated browser flavour: 84 Python tests and 23
